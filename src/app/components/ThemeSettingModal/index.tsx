...
import { zoomconvboxAtom, followArcThemeAtom, themeColorAtom } from '~app/state'
import { applyThemeMode, getDefaultThemeColor, applyBackgroundimageMode } from '~app/utils/color-scheme'
import { isArcBrowser } from '~app/utils/env'
import { ThemeMode, getUserThemeMode, setUserThemeMode } from '~services/theme'
import { BackgroundimageMode, getUserBackgroundimageMode, setUserBackgroundimageMode } from '~services/backgroundimage'
...
const ThemeSettingModal: FC<Props> = (props) => {
  ...
  const [backgroundimageMode, setBackgroundimageMode] = useState(getUserBackgroundimageMode())
  ...
  const setZoomconvboxWrapper = useCallback((val: boolean) => {
    setZoomconvbox(val)
    // trackEvent('change_theme_mode', { mode })
  }, [])

  const onBackgroundimageModeChange = useCallback((mode: BackgroundimageMode) => {
    setUserBackgroundimageMode(mode)
    setBackgroundimageMode(mode)
    console.log("onBackgroundimageModeChange", mode)
    applyBackgroundimageMode(mode)
    // trackEvent('change_backgroundimage_mode', { mode })
  }, [])
      ...
      <div className="p-5 pb-10 flex flex-col gap-5">
        ...
        {/*<div className="w-[300px]">
          <p className="font-bold text-lg mb-3">
            {t('Enable Zoom on Hover')}
            <ToggleSwitch 
              text=''
              checked={zoomconvbox}
              onChange={setZoomconvboxWrapper}
              disabled={!premiumState.activated}/>
          </p>
        </div>*/}
        <div className="w-[300px]">
          <p className="font-bold text-lg mb-3">{t('Chat Background')}</p>
          <Select
            options={[
              { name: t('Topography(Default)'), value: BackgroundimageMode.Topography },
              { name: t('SlantedStars'), value: BackgroundimageMode.SlantedStars },
              { name: t('Cutout'), value: BackgroundimageMode.Cutout },
              { name: t('DiagonalStripes'), value: BackgroundimageMode.Diagonalstripes },
              { name: t('FallingTriangles'), value: BackgroundimageMode.Fallingtriangles },
              { name: t('FloatingCogs'), value: BackgroundimageMode.Floatingcogs },
              { name: t('FormalInvitation'), value: BackgroundimageMode.Formalinvitation },
              { name: t('Hexagons'), value: BackgroundimageMode.Hexagons },
              { name: t('Hideout'), value: BackgroundimageMode.Hideout },
              { name: t('Jigsaw'), value: BackgroundimageMode.Jigsaw },
              { name: t('Jupiter'), value: BackgroundimageMode.Jupiter },
              { name: t('Lisbon'), value: BackgroundimageMode.Lisbon },
              { name: t('Overcast'), value: BackgroundimageMode.Overcast },
              { name: t('Pianoman'), value: BackgroundimageMode.Pianoman },
              { name: t('PieFactory'), value: BackgroundimageMode.Piefactory },
              { name: t('Squares'), value: BackgroundimageMode.Squares },
              { name: t('TinyCheckers'), value: BackgroundimageMode.Tinycheckers },
              { name: t('Wallpaper'), value: BackgroundimageMode.Wallpaper },
              { name: t('YYYPattern'), value: BackgroundimageMode.Yyy },
              { name: t('None'), value: BackgroundimageMode.None },
            ]}
