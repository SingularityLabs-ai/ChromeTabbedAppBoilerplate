...
import { MYAWESOMEAPP_EXTENSION_STORE } from '~app/consts'
    ...
    return (
      <Dialog title="🌟🌟🌟🌟🌟" open={open} onClose={() => setOpen(false)} className="rounded-2xl w-[600px]">
        <div className="flex flex-col items-center gap-4 py-6">
          <p className="font-semibold text-primary-text">{t('Enjoy MyAwesomeApp? Give us a 5-star rating!')}</p>
          <a
            href={MYAWESOMEAPP_EXTENSION_STORE}
            target="_blank"
            rel="noreferrer"
          >
      ...
      <Dialog title="🥳 🥳 🥳" open={open} onClose={() => setOpen(false)} className="rounded-2xl w-[600px]">
        <div className="flex flex-col items-center gap-4 py-6">
          <p className="font-semibold text-primary-text w-[300px] text-center">
            {t('You have opened MyAwesomeApp {{openTimes}} times, consider unlock all features?', { openTimes })}
          </p>
          <Link
            to="/premium"
