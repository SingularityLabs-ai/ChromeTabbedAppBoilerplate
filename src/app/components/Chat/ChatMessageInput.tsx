...
import promotEnggIcon from '~/assets/icons/prompt-engineering-black-thin.svg'
...
const ChatMessageInput: FC<Props> = (props) => {
  ...
  const { placeholder = t('Input placeholder is here') } = props
...
// }, [props, value])
  ...
  const openPromptLibrary = useCallback(() => {
    ...
    // trackEvent('open_prompt_library')
          ...
          // <IconButton icon={promotEnggIcon} tooltipText="Browse Prompts" className="bg-[#00000014] dark:bg-[#ffffff26]" onClick={openPromptLibrary}/>
  ...
  return (
    <form className={cx('flex flex-row items-center gap-3', props.className)} onSubmit={onFormSubmit} ref={formRef}>
      ...
      <div className="h-full w-full flex flex-col justify-center" ref={refs.setReference} {...getReferenceProps()}>
        <TextInput
          ref={inputRef}
          formref={formRef}
          name="input"
